...
$fetch_rows = mysql_num_rows($conn,$query);
if ( $fetch_rows > 0 )
{
  echo "Then Do something";
} else {
     mysql_query("insert into userdata1(phone,user_id,first_name,last_name,b_add,gstin) values('$phone','$user_id','$first_name', '$last_name', '$b_add', '$gstin')",$conn) or die(mysql_error());

    echo "<script>alert('Successfully Recorded, First penalty: 100php!')</script>";
    header("Refresh: 0;url=admin_publicviolation.php");
}/** */

/*$re = mysql_error($conn,"SELECT phone from userdata1 where phone = '$phone'");
if($re > 0)
{echo "try again";}
else{echo "we have to insert0";} 
mysqli_close($conn);/** */
$phone = $_REQUEST['phone'];
$query_login = "SELECT phone FROM userdata1 WHERE phone='$phone';";
$sql = "INSERT INTO userdata1 VALUES ('$phone','$first_name',
			'$last_name','$company','$b_add','$gstin')";
...
if($anything_found>0)
    {
    echo "Sorry, that number is already taken. Please choose another.\n";
   }
